...
async fn main(_spawner: Spawner) {
    ...
    sensor.tap_config_set(tap_cfg).unwrap();
    ...
    loop {
        ...
        if status.single_tap == 1 {
            writeln!(&mut msg, "single TAP detected").unwrap();
        }
        if status.double_tap == 1 {
            writeln!(&mut msg, "double TAP detected").unwrap();
        }
        if status.triple_tap == 1 {
            writeln!(&mut msg, "triple TAP detected").unwrap();
        }
