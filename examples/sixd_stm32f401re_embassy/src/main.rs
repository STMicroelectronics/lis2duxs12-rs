...
use lis2duxs12_rs::{I2CAddress, Lis2duxs12, ID};
...
async fn main(_spawner: Spawner) {
    ...
    sensor.sixd_config_set(sixd_config).unwrap();
    ...
    loop {
        ...
        if sixd_event & 0x40 != 0 {
            if sixd_event & 0x8 != 0 {
                writeln!(&mut msg, "Y-axis UP").unwrap();
            } else if sixd_event & 0x01 != 0 {
                writeln!(&mut msg, "X-axis DOWN").unwrap();
            } else if sixd_event & 0x02 != 0 {
                writeln!(&mut msg, "X-axis UP").unwrap();
            } else if sixd_event & 0x04 != 0 {
                writeln!(&mut msg, "Y-axis DOWN").unwrap();
            } else if sixd_event & 0x20 != 0 {
                writeln!(&mut msg, "Z-axis UP").unwrap();
            } else if sixd_event & 0x10 != 0 {
                writeln!(&mut msg, "Z-axis DOWN").unwrap();
            }
